...
function readTransactions_() {
  ...
  const values = sheet.getDataRange().getDisplayValues();
  return values.slice(1).filter(function(r){ return r[0]; }).map(function(r){
    return {id:r[0], type:r[1], qty:r[2] ? Number(r[2]) : null, price:r[3] ? Number(r[3]) : 0, total:r[4] ? Number(r[4]) : 0, note:r[5] || '', date:r[6] || '', by:r[7] || ''};
  });
}
...
function addTransaction_(body, email) {
  ...
  const sheet = SpreadsheetApp.openById(SHEET_ID).getSheetByName(TX_SHEET);
  sheet.appendRow([Utilities.getUuid(), type, type === 'ad' ? 1 : qty, price, total, clean_(body.note,120), new Date(), email]);
}
...
function setupHeaders_() {
  ...
  if (tx.getRange('A1:H1').getDisplayValues()[0].join('|') !== 'ID|Type|Quantity|Unit Price|Total|Note|Occurred At|Created By') tx.getRange('A1:H1').setValues([['ID','Type','Quantity','Unit Price','Total','Note','Occurred At','Created By']]);
}
